...
BEGIN
  ...
  -- a unique top-level node for mRNA data
	...
	--	prevent that records are dropped if normalizedreadcount = 0
	update tm_lz.lt_src_rnaseq_data
	set normalizedreadcount = NULL
	where dataType = 'R' and normalizedreadcount = 0;
	...
	select gs.region_id
		  ...
		  ,avg(md.normalizedreadcount::double precision)
		  ,sd.patient_id
		  ,TrialId
		  ,sd.assay_id
	...
	where sd.sample_cd = md.expr_id
	  ...
	  and case when dataType = 'R'
			   then case when md.normalizedreadcount is NULL or md.normalizedreadcount > 0 then 1 else 0 end
	...
	if dataType = 'T' then
		sqlText := 'insert into ' || partitionName || ' (partition_id, region_id, assay_id, patient_id, trial_name, readcount, zscore) ' ||
				   ...
				   'case when normalizedreadcount < -2.5 then -2.5 when normalizedreadcount > 2.5 then 2.5 else normalizedreadcount end ' ||
		...
		select   region_id
			...
			,case when dataType = 'R'
			   then normalizedreadcount
			   else case when logBase = -1 then 0 else power(logBase,normalizedreadcount) end
			 end
			,case when dataType = 'L' then normalizedreadcount else log(logBase,normalizedreadcount::numeric) end
			,patient_id 
			,trial_name
		from tm_wz.wt_subject_rnaseq_region;		
		get diagnostics rowCt := ROW_COUNT;
		...
		sqlText := 'insert into ' || partitionName || ' (partition_id, region_id, assay_id, patient_id, trial_name, readcount, normalizedreadcount, lognormalizedreadcount, zscore) ' ||
