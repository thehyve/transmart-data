--
-- Type: PROCEDURE; Owner: TM_CZ; Name: RWG_LOAD_HEAT_MAP_RESULTS
--
...
begin
  ...
  END IF;
	...
	--	changed to use sql instead of view, view pulled back all studies   20121203 JEA
	 ...
	 insert into  /*+ APPEND */ biomart.heat_map_results
	 (subject_id
	 ,log_intensity
	 ,cohort_id
	 ,probe_id
     ...
     select /*+ PARALLEL */ replace(replace(pd.sourcesystem_cd,xref.study_id,''),':','') as subject_id
			,md.log_intensity
			,cex.cohort_id
			,dma.probeset
