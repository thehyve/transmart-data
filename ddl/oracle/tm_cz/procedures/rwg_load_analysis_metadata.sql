--
-- Type: PROCEDURE; Owner: TM_CZ; Name: RWG_LOAD_ANALYSIS_METADATA
--
...
/***************************************************************/
-- LOOP FOR SAMPLE_TYPE

 Select NVL(Max(Length(Regexp_Replace(cohort.sample_type,'[^;]'))),0)+1
 Into Dcount
 From TM_LZ.Rwg_Cohorts Cohort
 Where upper(Cohort.Study_Id)=upper(trialID);
  ...
  for lcount in 1 .. dcount
      ...
      STEPCT := STEPCT + 1;
      cz_write_audit(jobId,databaseName,procedureName,'Starting COHORT SAMPLE_TYPE LOOP, pass: ',lcount,stepCt,'Done');

    -- SAMPLE_TYPE: check for any records that do not have a match in the taxonomy
    Insert Into Cz_Rwg_Invalid_Terms (Study_Id, Category_Name, Term_Name)
     Select distinct upper(Cohort.Study_Id), 'SAMPLE_TYPE', trim(Parse_Nth_Value(cohort.sample_type,lcount,';'))
    From TM_LZ.Rwg_Cohorts Cohort
    Where upper(Cohort.Study_Id)=upper(trialID)
    and Not Exists 
    (Select Upper(Tax.Term_Name) From Searchapp.Search_Taxonomy Tax
    Where Upper(trim(Parse_Nth_Value(cohort.sample_type,lcount,';'))) = Upper(Tax.Term_Name))
  And Trim(Parse_Nth_Value(cohort.sample_type,lcount,';')) Is Not Null;


    -- SAMPLE_TYPE: insert terms into attribute table
    Insert Into Biomart.Bio_Analysis_Attribute (Study_Id, Bio_Assay_Analysis_Id, Term_Id, Source_Cd)
    Select Distinct upper(Cohort.Study_Id), Xref.Bio_Assay_Analysis_Id, Tax.Term_Id, Upper('SAMPLE_TYPE:' ||trim(Parse_Nth_Value(cohort.sample_type,lcount,';'))) 
    From  TM_LZ.Rwg_Cohorts Cohort, Biomart.Bio_Analysis_Cohort_Xref Xref, Searchapp.Search_Taxonomy Tax
    Where upper(Cohort.Cohort_Id) = upper(Xref.Cohort_Id)
    And upper(Xref.Study_Id) = upper(Cohort.Study_Id)
    And Upper(Trim(Parse_Nth_Value(cohort.sample_type,Lcount,';'))) = Upper(Tax.Term_Name)
    and upper(cohort.study_id) =upper(trialID); 


      STEPCT := STEPCT + 1;
      Cz_Write_Audit(Jobid,Databasename,Procedurename,'Insert SAMPLE_TYPE into Bio_Analysis_Attribute (LOOP)',Sql%Rowcount,Stepct,'Done');
      commit;	
  ...
  End Loop;
      ...
      Stepct := Stepct + 1;
      Cz_Write_Audit(Jobid,Databasename,Procedurename,'END SAMPLE_TYPE LOOP',Sql%Rowcount,Stepct,'Done');
      Commit;	
-- END SAMPLE_TYPE LOOP
/***************************************************************/
...
and upper(ext.study_id) =upper(trialID); 
    ...
    SELECT BAA.BIO_ANALYSIS_ATTRIBUTE_ID, SKL.ANCESTOR_ID, SKL.SEARCH_KEYWORD_ID 
    FROM searchapp.solr_keywords_lineage skl, biomart.bio_analysis_attribute BAA
    WHERE SKL.TERM_ID = BAA.TERM_ID
    AND upper(baa.study_id) = upper(trialID);
